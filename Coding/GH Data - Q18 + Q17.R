...
# Results of Q17
  # Responses to whether all pre-medical students should take at least one global health-focused class before entering med school (Q17)
  df17 <- df %>% 
    filter(Q17 != "") %>% view()
  ...
  df17 %>%
    ...
    arrange(factor(Q17, levels = c('Strongly Agree', 'Agree', 'Undecided', 'Disagree', 'Strongly Disagree'))) %>% ggplot(aes(Q17, Counts)) +
    geom_bar(stat = "identity", fill = 'cornsilk3') +
    labs(title = "Should Pre-Med Students Take a Global Health Course?", x = "Responses") +
    geom_text(aes(label = signif(Counts)), nudge_y = 2)
  
# How do the results of whether all pre-medical students should take at least 
# one global health-focused course before entering med school (Q17) correlate with
# stated interest in global health (Q14)
  df17 %>%
    filter(Q14 != "") %>% 
    group_by(Q14, Q17) %>%
    summarise(
    ...
    ggplot(aes(Q14, Counts, fill = Q17)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = "Take Undergraduate Global Health Course by Global Health Career Interest", x = "Global Health Career Interest",
         fill = "Should pre-Meds Take a Global Health Course:") +
    theme(legend.position = "bottom") +
    geom_text(aes(label = signif(Counts)), vjust = -.1, position = position_dodge(width = .9))
  
# How do the results of whether all pre-medical students should take at least
# one global health-focused course before entering med school (Q17) correlate
# with those who lived outside of the US (Q12)
  df17 %>%
    filter(Q12 != "") %>% 
    group_by(Q12, Q17) %>%
    summarise(
    ...
    ggplot(aes(Q12, Counts, fill = Q17)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = "Take Undergraduate Global Health Course by Lived Outside US", x = "Lived Outside US",
         fill = "Should pre-Meds Take a Global Health Course:") +
    theme(legend.position = "bottom") +
    geom_text(aes(label = signif(Counts)), vjust = -.1, position = position_dodge(width = .9))
  
# How do the results of whether all pre-medical students should take at least one global health-focused course before entering med school (Q17) correlate with major (Q6)
  df17 %>%
    filter(Q6 != "") %>% 
    group_by(Q6, Q17) %>%
    summarise(
    ...
    ggplot(aes(Q6, Counts, fill = Q17)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = "Take Undergraduate Global Health Course by Major", x = "Major",
         fill = "Should pre-Meds Take a Global Health Course:") +
    theme(legend.position = "bottom") +
    geom_text(aes(label = signif(Counts)), vjust = -.1, position = position_dodge(width = .9))
